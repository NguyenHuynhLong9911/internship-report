---
title: "Week 10 Worklog"
date: 2026-06-15
weight: 10
chapter: false
pre: " <b> 1.10. </b> "
---

### Week 10 Objectives

* Investigate and resolve the Amazon Cognito `No Token` error.
* Investigate the payment error encountered when purchasing a domain through Amazon Route 53.

### Tasks completed this week

| No. | Task | Period | Result |
| --- | ---- | ------ | ------ |
| 1 | Review the Cognito configuration and the frontend sign-in and token flow | 15/06–18/06 | Identified and adjusted the authentication flow related to the `No Token` error |
| 2 | Review the payment method and attempt domain registration through Route 53 | 19/06–21/06 | Could not purchase the domain and began considering the default Amplify domain |

### Week 10 Achievements

* Improved the Cognito authentication flow and addressed token-related issues.
* Documented the Route 53 payment limitation and prepared an alternative domain approach.
